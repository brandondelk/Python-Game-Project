...
# Load heal sprite sheet
heal_sheet = pygame.image.load('heal.png').convert_alpha()
heal_width, heal_height = 32, 32
...
# Load health images
health_images = [pygame.image.load('health3.png').convert_alpha() for i in range(1, 4)]
...
meteor_speed = 7
meteor_speed_boost = 0  # Variable to track meteor speed boost
meteor_speed_boost_timer = pygame.time.get_ticks()  # Timer for meteor speed boost
...
# Alien variables
alien_speed = 2
alien_spawn_delay = 8000  # Spawn delay for aliens in seconds
...
# Pink square variables
pink_square_width, pink_square_height = 32, 32
pink_square_speed = 2  # Adjust speed as needed
pink_squares = []  # List to store active pink squares
last_pink_square_spawn = pygame.time.get_ticks()  # Variable to track last pink square spawn time
...
pause_start_time = 0  # Variable to store the time when the game is paused
...
resume_text = pause_font.render("Resume", True, WHITE)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            elif event.key == pygame.K_p:  # Pause the game when 'p' key is pressed
                paused = not paused
                if paused:
                    pause_start_time = pygame.time.get_ticks()  # Record the time when the game is paused
                else:
                    # Adjust the start time to compensate for the pause duration
    ...
    if paused:
        # Draw "Paused" message and resume button
    ...
    for enemy in enemies[:]:
        enemy[0] -= (meteor_speed + meteor_speed_boost)  # Apply boosted speed
        ...
        if enemy[0] < player_x + player_width and enemy[0] + player_width > player_x and \
           enemy[1] < player_y + player_height and enemy[1] + player_height > player_y:
            ...
            if player_health <= 0:
                ...
                # Reset player health
                player_health = 3
                # Reset meteor speed boost
                meteor_speed_boost = 0
                meteor_speed_boost_timer = pygame.time.get_ticks()
            else:
    ...
    for i in range(player_health):
        screen.blit(health_images[i], (10 + i * (health_width + 5), 10))
    ...
    # Spawn heal sprite every 1 minute
    if current_time - last_pink_square_spawn >= 60000:
        heal_y = random.randint(0, HEIGHT - heal_height)
        pink_squares.append([WIDTH, heal_y])
        last_pink_square_spawn = current_time

    # Move and draw heal sprites
    for heal_sprite in pink_squares[:]:
        heal_sprite[0] -= pink_square_speed
        # Animate heal sprite sheet
        heal_frame_count = 4
        heal_animation_speed = 150
        heal_current_frame = (current_time // heal_animation_speed) % heal_frame_count
        heal_rect = pygame.Rect(heal_current_frame * heal_width, 0, heal_width, heal_height)
        screen.blit(heal_sheet, (heal_sprite[0], heal_sprite[1]), heal_rect)

        # Collision detection with player
        if (player_x - heal_width < heal_sprite[0] < player_x + player_width and
                player_y - heal_height < heal_sprite[1] < player_y + player_height):
            # Recover 1 health if player health is not full
            if player_health < 3:
                player_health += 1
            pink_squares.remove(heal_sprite)
        # Remove heal sprite if it goes off the left side of the screen
        if heal_sprite[0] < 0:
            pink_squares.remove(heal_sprite)
    ...
    # Check if it's time to boost the meteor speed
    if current_time - meteor_speed_boost_timer >= 20000:  # Boost every 20 seconds
        meteor_speed_boost += 1  # Increase meteor speed by 1
        meteor_speed_boost_timer = current_time  # Reset the timer
